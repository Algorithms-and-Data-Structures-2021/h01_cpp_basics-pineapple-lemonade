...
ResizeStorageStatus resize_storage(Book *&storage, int size, int new_capacity) {
    ...
    Book * tempStorage = new Book[new_capacity]{};
    std::copy(storage,storage + size, tempStorage);
    delete[] storage;
    storage = new Book[new_capacity]{};
    std::copy(tempStorage,tempStorage + size,storage);
    delete[] tempStorage;
    tempStorage = nullptr;
...
void BookStore::AddBook(const Book &book) {
    if (storage_size_ == storage_capacity_) {
        ...
        ResizeStorageStatus status;
        status = resize_storage_internal(storage_capacity_ + kCapacityCoefficient);//если често не совсем понял зачем обрабатывать,
        //поэтому и не обрабатываю
    }
